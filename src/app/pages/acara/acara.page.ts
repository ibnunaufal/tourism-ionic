...
import { AcaraDetailPage } from '../acara-detail/acara-detail.page';
import { DetailPage } from '../detail/detail.page';

@Component({
  selector: 'app-acara',
  templateUrl: './acara.page.html',
  styleUrls: ['./acara.page.scss'],
})
export class AcaraPage implements OnInit {
  ...
  search(kata){
    ...
    this.arr = [];
    this.dataService
      .getAcaraPage(this.start, this.limit, this.kata, )
      .then((response: any) => {
  ...
  getData(start, limit) {
    ...
    this.arr = [];
    this.dataService
      .getAcaraPage(start, limit, this.kata, this.tag)
      .then((response: any) => {
        this.isLoading = false
        console.log(response);
        this.arr = response.body.data;
  ...
  loadData(event) {
    ...
    this.dataService.getAcaraPage(this.start, this.limit, this.kata, this.tag).then(
      (response: any) => {
  ...
  doRefresh(event) {
    ...
    this.dataService
      .getAcaraPage(this.start, this.limit, this.kata, this.tag)
      .then((response: any) => {
        this.isLoading = false
        this.arr = response.body.data;
  ...
  sanitizeImg(url){
    let s = this.apiUrl + 'img/acara/' + url
    return s
  }
